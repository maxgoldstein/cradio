...
struct IEEE80211_mac_frame {
    ...
    uint32_t fcs; // FCS: frame check sequence (crc)
};
typedef struct IEEE80211_mac_frame IEEE80211_generic_t;
...
/* compute crc of inp: input packet */
uint32_t calc_crc(uint8_t *inp, uint32_t len);

/* convert MAC address string to a char array
 * input:
 *  inp: uint8_t* - MAC address as string (FF:FF:FF:FF:FF:FF)
 * output:
 *  outp: uint8_t* - MAC address as bytes (\xff\xff\xff\xff\xff\xff)
 */
void mac_to_bytes(uint8_t *inp, uint8_t *outp);

/* converts uint16_t (u_short) from host endianness to little endian */
uint16_t htoles(uint16_t v);

/* convert IEEE80211 MAC frame to byte array */
